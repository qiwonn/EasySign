...
/** 列表视图适配器 */
public final class SignAdapterX extends BaseAdapter {
	/**
    ...
    public SignAdapterX(List<SignBeanX> mData, Context context) {
        this.mData = mData;
	...
	public void allEdited(){
		if(mData.size()<=0)return;
		for(SignBeanX d : mData)
			d.cbvisibility = 0x0;
	}
	
	public void noneEdited(){
		if(mData.size()<=0)return;
		for(SignBeanX d : mData)
			d.cbvisibility = 0x08;
	}
