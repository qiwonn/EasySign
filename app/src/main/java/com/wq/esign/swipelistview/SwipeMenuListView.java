...
import com.wq.esign.SignManager;
/** 自定义带可滑动菜单的列表视图 */
public class SwipeMenuListView extends ListView {
    ...
    public void setAdapter(ListAdapter adapter) {
        super.setAdapter(new SwipeMenuAdapter(getContext(), adapter) {
				@Override
				public void onItemClick(SwipeMenuView view,int index) {
					boolean flag = false;
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (view instanceof SwipeMenuLayout) {
                    //如果有打开了 且触点不在当前menuitem上 就拦截.
                    if (mTouchView != null && mTouchView.isOpen() && !inRangeOfView(mTouchView.getMenuView().getView(), ev)) {
						smoothCloseMenu();
                        return true;
                    }
